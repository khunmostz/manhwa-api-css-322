...
app.get("/api/category/fcmanhwa", (req, res) => {
  ...
  con.query("SELECT * FROM cate_manhwa ", (err, result) => {
    if (err) {
      console.log(err);
    }
    res.json(result);
...
app.get("/api/category/:id/fcmanhwa", (req, res) => {
  ...
  con.query(
    "SELECT * FROM cate_manhwa INNER JOIN manhwa_list ON cate_manhwa.cate_id = manhwa_list.cate_id AND manhwa_list.cate_id = (?)",
    [cate_id],
    (err, result) => {
      if (err) {
        console.log(err);
...
app.put("/api/update/manhname/fcmanhwa/:id", (req, res) => {
  ...
  con.query(sql, [manh_name, manh_id], (err, result) => {
    if (err) {
